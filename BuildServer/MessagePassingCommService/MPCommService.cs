/////////////////////////////////////////////////////////////////////
//  MPCommService.cs - Communication Services are described        //
//  ver 1.0                                                        //
...
namespace MessagePassingComm
{
    ...
    public class Receiver : IMessagePassingComm
    {
        ...
        {
            Console.WriteLine("Requirement 2: Shall include a Message-Passing Communication Service built with WCF");
        ...
        public bool openFileForWrite(string name,string recieverpath)
        {
            try
            {
                string writePath = Path.Combine(recieverpath, name);
    ...
    public class Sender
    {
        ...
        public bool postFile(string fileName,string senderpath,string recieverpath)
        {
            ...
            try
            {
                string path = Path.Combine(senderpath, fileName);
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(fileName,recieverpath);
                while (true)
                {
